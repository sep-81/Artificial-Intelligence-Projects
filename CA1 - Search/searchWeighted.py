...
def cal_heuristic(state: State, morids, all_recipes, alpha):
    ...
    return len(must_visit_nodes) * alpha
...
def Astar(graph: Graph, hard_nodes, morids_recipes, start_node, all_recipes, morids):
  ...
  while(True):
    ...
    for neighbor in graph.graph[cur_state.position]:
      ...
      next_state.est_time = cal_heuristic(next_state, morids, all_recipes, alpha) + next_state.current_time  
